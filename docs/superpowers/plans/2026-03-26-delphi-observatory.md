# Delphi Observatory Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Add a `/delphi-observe` command that renders deliberation dockets as issue-threaded, commentary-rich HTML pages in the visualizer.

**Architecture:** Single command file (`commands/delphi-observe.md`) that reads docket directories, transforms phase-oriented files into issue-oriented threads, generates HTML with AI commentary, and pushes to the visualizer MCP. Live mode polls for file changes; post-hoc mode renders once. No new skills, agents, or templates.

**Tech Stack:** Claude Code command (Markdown/YAML), Visualizer MCP (push_screen), inline HTML/CSS generation.

**Spec:** `docs/superpowers/specs/2026-03-25-delphi-observatory-design.md`

---

### Task 1: Command Frontmatter and Argument Parsing

**Files:**
- Create: `commands/delphi-observe.md`

This task creates the command file with its YAML frontmatter and argument parsing logic — the entry point that determines whether we're in post-hoc or live mode.

- [ ] **Step 1: Create the command file with frontmatter and argument parsing**
...
Run: `cat commands/delphi-observe.md | head -5`
Expected: The YAML frontmatter with `description`, `allowed-tools`, and `argument-hint` fields.

- [ ] **Step 3: Commit**

```bash
git add commands/delphi-observe.md
git commit -m "feat: add /delphi-observe command — argument parsing and mode detection"
```

---

### Task 2: Docket Reading and Issue Threading Logic

**Files:**
- Modify: `commands/delphi-observe.md`

This task adds the instructions for reading all docket files and transforming them into issue threads. This is the intellectual core of the observatory.

- [ ] **Step 1: Add the docket reading section**

Append after the argument parsing section in `commands/delphi-observe.md`:
...
Run: `grep "^## " commands/delphi-observe.md`
Expected: Three top-level sections: `## Parse arguments`, `## Read the docket`, `## Build issue threads`

- [ ] **Step 3: Commit**

```bash
git add commands/delphi-observe.md
git commit -m "feat: delphi-observe — docket reading and issue threading logic"
```
...
This task adds the HTML generation instructions — the full layout spec that Claude follows to build the observatory page.

- [ ] **Step 1: Add the HTML generation section**

Append after the issue threading section in `commands/delphi-observe.md`:
...
Run: `grep "^## " commands/delphi-observe.md`
Expected: Four top-level sections: `## Parse arguments`, `## Read the docket`, `## Build issue threads`, `## Generate the observatory HTML`

- [ ] **Step 3: Commit**

```bash
git add commands/delphi-observe.md
git commit -m "feat: delphi-observe — full HTML generation with layout, styles, and threading"
```

---

### Task 4: Commentary System

**Files:**
- Modify: `commands/delphi-observe.md`

This task adds the detailed instructions for how Claude generates commentary — the three voices, placement rules, and mode-specific behavior.

- [ ] **Step 1: Add the commentary section**

Append after the HTML generation section in `commands/delphi-observe.md`:
...
```bash
git add commands/delphi-observe.md
git commit -m "feat: delphi-observe — commentary system with three voices and placement rules"
```

---

### Task 5: Live Mode Polling and Session Management

**Files:**
- Modify: `commands/delphi-observe.md`

This task adds the visualizer session lifecycle (launch, push, close) and the live-mode polling loop.

- [ ] **Step 1: Add the execution section**

Append after the commentary section in `commands/delphi-observe.md`:
...
```bash
git add commands/delphi-observe.md
git commit -m "feat: delphi-observe — visualizer session management and live polling loop"
```
...
This task updates the project documentation to reflect the new command.

- [ ] **Step 1: Update CLAUDE.md implementation status**

Add to the Implementation Status section in `CLAUDE.md`, after the tone system line:
...
- [ ] **Step 2: Verify the complete command file structure**

Run: `grep "^## " commands/delphi-observe.md`
Expected output (6 top-level sections):
```
## Parse arguments
## Read the docket
## Build issue threads
...
- [ ] **Step 3: Verify the command file has proper frontmatter**

Run: `head -5 commands/delphi-observe.md`
Expected: YAML frontmatter block with `description`, `allowed-tools`, and `argument-hint`.

- [ ] **Step 4: Commit**

```bash
git add CLAUDE.md commands/delphi-observe.md
git commit -m "docs: add observatory to CLAUDE.md implementation status"
```
...
This task verifies the observatory works against a real or simulated docket.

- [ ] **Step 1: Check for existing docket directories**

Run: `ls .deliberation/dockets/ 2>/dev/null || echo "No dockets found"`

If dockets exist, pick one for testing. If none exist, create a minimal test docket:

```bash
mkdir -p .deliberation/dockets/20260326-test-observatory
```

Then create minimal test files:
- `proposition.md` with a sample proposition
- `positions/round-1/proposer.md` with a sample position
- `challenges/round-1.md` with a sample challenge
- `responses/round-1/proposer.md` with a sample response including `[ACTION: DEFEND]` and `[CITE: test.md, section]`
- `synthesis/round-1.md` with a settled point
- `decision.md` with a ratified outcome
- `docket.json` with minimal metadata

- [ ] **Step 2: Run the observatory in post-hoc mode**

Invoke: `/delphi-observe .deliberation/dockets/{test-docket-path}/`

Verify:
- A browser tab opens with the observatory
- The header shows the proposition, mode, and outcome
- Issue threads are displayed with position → challenge → response flow
- Commentary appears in the Deliberation Pulse and notable thread asides
- The timeline breadcrumb shows all phases as completed
- Threads are collapsible (click to toggle)

- [ ] **Step 3: Clean up test docket if created**

If a test docket was created in Step 1:
```bash
rm -rf .deliberation/dockets/20260326-test-observatory
```
...
```bash
git add commands/delphi-observe.md
git commit -m "fix: delphi-observe adjustments from integration testing"
```
